                    ...
                    if isinstance(node, ast.ImportFrom):
                        ...
                        if module and module.startswith('oure.'):
                            ...
                            if layer == 'core' and imported_layer != 'core':
                                pytest.fail(f"Core layer {file} imports from {imported_layer}")

                            # Other layers: can only import from themselves or core
                            if layer != 'core' and imported_layer not in (layer, 'core'):
                                ...
                                if layer in ('conjunction', 'uncertainty') and imported_layer == 'physics':
                                    continue
                                pytest.fail(f"Layer {layer} in {file} imports from {imported_layer}")
