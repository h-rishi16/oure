...
def test_maneuver_propagator_backward():
    ...
    state = StateVector(
        r=np.array([7000, 0, 0]),
        v=np.array([0, 7.5, 0]),
        epoch=burn_epoch + timedelta(minutes=10),
        sat_id="1",
    )
...
def test_breakup_model_randomness():
    ...
    s2 = StateVector(
        r=np.array([7000, 0, 0]), v=np.array([0, -7.5, 0]), epoch=s1.epoch, sat_id="2"
    )

    f1 = BreakupModel.simulate_collision(
        s1, 500, s2, 500, s1.epoch, num_fragments=10, random_seed=42
    )
    f2 = BreakupModel.simulate_collision(
        s1, 500, s2, 500, s1.epoch, num_fragments=10, random_seed=42
    )
