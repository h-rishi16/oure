...
class CDMParser:
    ...
    def parse_json(filepath: str) -> ConjunctionEvent:
        ...
        def _parse_state_cov(
            ...
            sat_id = str(meta.get("OBJECT_DESIGNATOR", "UNKNOWN"))

            state_dict = data_sec.get("state_vector", {})
            r = np.array(
                [
                    state_dict.get("X", 0.0),
                    state_dict.get("Y", 0.0),
                    state_dict.get("Z", 0.0),
                ]
            )
            v = np.array(
                [
                    state_dict.get("X_DOT", 0.0),
                    state_dict.get("Y_DOT", 0.0),
                    state_dict.get("Z_DOT", 0.0),
                ]
            )
            ...
            if np.all(cov == 0):
                ...
                logging.getLogger("oure.data.cdm_parser").warning(
                    f"CDM for {sat_id} has no covariance — using default 1 km sigma. Pc unreliable."
                )
