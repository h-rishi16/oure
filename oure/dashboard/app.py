...
st.markdown(
    "Monitor high-risk conjunction events and analyze Conjunction Data Messages (CDMs) in real-time."
)
...
if page == "Live Fleet Status":
    ...
    try:
        ...
        if data:
            ...
            col3.metric(
                "YELLOW Alerts (Pc > 1e-5)", len(df[df["warning_level"] == "YELLOW"])
            )

            st.subheader("High-Risk Encounter Feed")

            # Display styled dataframe
            def color_risk(val: Any) -> str:
                color = (
                    "red" if val == "RED" else "yellow" if val == "YELLOW" else "green"
                )
        ...
        st.warning(
            "No live data found. Run `oure analyze` or `oure monitor` to generate results."
        )
...
# --- PAGE 2: CDM Analysis Tool ---
    ...
    st.markdown(
        "Upload official Space Force CDMs to calculate exact collision probabilities using Foster's algorithm."
    )

    uploaded_file = st.file_uploader("Upload JSON CDM", type=["json"])
    hbr = st.number_input(
        "Combined Hard-Body Radius (meters)", min_value=1.0, value=20.0
    )
                ...
                try:
                    ...
                    with col2:
                        st.metric(
                            ...
                            res.conjunction.tca.strftime("%Y-%m-%d %H:%M:%S UTC"),
                        )
                        st.metric(
                            "Miss Distance",
                            f"{res.conjunction.miss_distance_km:.3f} km",
                        )

                    if res.warning_level == "RED":
                        st.error(
                            "EMERGENCY: Collision threshold exceeded. Recommend immediate avoidance maneuver analysis."
                        )

                    st.subheader("Encounter Visualizations")
                    tab1, tab2 = st.tabs(
                        ["3D Orbital Geometry", "2D B-Plane Projection"]
                    )

                    with tab1:
                        st.markdown(
                            "Interactive 3D representation of the encounter around TCA (±30 seconds)."
                        )
                    ...
                    with tab2:
                        st.markdown(
                            "2D projection of the encounter onto the B-Plane (orthogonal to relative velocity)."
                        )
...
# --- PAGE 3: Risk Evolution History ---
    ...
    st.markdown(
        "Track how the Probability of Collision (Pc) evolves over time as TCA approaches."
    )
    ...
    if st.button("Load History"):
        try:
            ...
            if not records:
                st.warning(
                    f"No historical risk data found for {primary_id} vs {secondary_id}."
                )
            else:
                ...
                fig.add_hline(
                    ...
                    line_dash="dash",
                    line_color="red",
                    annotation_text="RED Alert (1e-3)",
                )
                fig.add_hline(
                    y=1e-5,
                    line_dash="dash",
...
# --- PAGE 4: Background Task Manager ---
    ...
    st.markdown(
        "Submit massive O(N log N) KD-Tree fleet screening jobs to the Celery background worker queue."
    )
    ...
    with col2:
        secondary_ids_raw = st.text_area(
            "Secondary IDs (comma separated)", value="43205,47813,40379"
        )
    ...
    if st.button("Submit Screening Job"):
        ...
        try:
            ...
            st.error(
                f"Failed to submit task. Is the FastAPI backend running? Error: {e}"
            )
    ...
    # Simple polling UI
    task_to_check = st.text_input(
        "Enter Task ID to check status",
        value=st.session_state.get("current_task_id", ""),
    )
    if st.button("Check Status") and task_to_check:
        try:
            task_res = requests.get(f"http://localhost:8000/tasks/{task_to_check}")
            ...
            if state == "PENDING":
                ...
                st.warning(
                    f"Task is running: {data.get('meta', {}).get('status', 'Processing...')}"
                )
            elif state == "SUCCESS":
            ...
            st.error(
                f"Failed to fetch task status. Is the FastAPI backend running? Error: {e}"
            )
