...
class SGP4Propagator(BasePropagator):
    ...
    def propagate_to(self, state: StateVector, target_epoch: datetime) -> StateVector:
        ...
        return StateVector(
            r=np.array(r), v=np.array(v), epoch=target_epoch, sat_id=state.sat_id
        )
    ...
    def _propagate_many_simplified(
        self, states: np.ndarray, initial_epoch: datetime, target_epoch: datetime
    ) -> np.ndarray:
        # Simplified SGP4-like logic for state perturbations (Monte Carlo)
        n0 = (
            self.tle.mean_motion_rev_per_day
            * constants.TWO_PI
        ...
        _, ecc, inc_deg, raan_deg, omega_deg, nu0 = rv2coe_vectorized(
            states[:, :3], states[:, 3:]
        )
        ...
        omega = (
            omega0
            + j2_factor * n_tle * (2.5 * np.cos(inc) ** 2 - 0.5) * dt_since_initial
        )
        ...
        r_vecs, v_vecs = self._elements_to_eci_vectorized(
            a_tle, ecc, inc, raan, omega, nu
        )

        return np.hstack([r_vecs, v_vecs])

    def _elements_to_eci_vectorized(
        self,
        a: np.ndarray | float,
        e: np.ndarray,
        i: np.ndarray,
    ...
    def _rot_pqw_to_eci_vectorized(
        self, raan: np.ndarray, i: np.ndarray, omega: np.ndarray
    ) -> np.ndarray:
